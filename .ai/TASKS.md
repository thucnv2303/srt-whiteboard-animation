...
- `TASK-026` — Sửa triệt để lỗi đè chữ/icon với Auto-Fit Padding, thiết lập Sổ bộ Chủ đề Đã Duyệt và Chuẩn hóa Toàn bộ Kế hoạch theo Golden Benchmark (Slot 1 & 2):
...
- `TASK-027` — Xây dựng Động cơ Sản xuất Phổ quát cho Toàn bộ 330 Slot (`whiteboard_app/slot_builder.py`) & Làm giàu Dày đặc Hệ thống Icon Vector theo Kịch bản:
  + Phát triển module `whiteboard_app/slot_builder.py` và công cụ dòng lệnh `scripts/generate_lively_slot.py`: Đóng gói trọn vẹn pipeline Tiêu Chuẩn Vàng (Golden Benchmark) cho phép tạo tự động bất kỳ slot nào từ kế hoạch 330 video (`CONTENT_PLAN_30_DAYS_330_VIDEOS.csv`) chỉ với một lệnh CLI (`--day D --slot S`).
  + Nâng cấp hệ thống minh họa trực quan phong phú: Bổ sung dày đặc các icon vector hình học thuần túy (vẽ bằng thuật toán, 100% loại bỏ ô vuông glyph `▯` trên Windows): Cân tiểu ly 10g, Cốc đong nước 100ml, Đồng hồ hẹn giờ 45 phút, Lưới rây mịn 0.5mm miết 2 lần, Biểu tượng cấm đỏ y khoa, Huy hiệu so sánh quả trứng dạ dày 30-50ml, Thìa nhỏ 5ml, Mặt trời cữ sáng 9h-10h, Lịch theo dõi dị ứng 3 ngày và Mây khí trướng bụng.
  + Tẩy sạch 100% ghost text trên các ảnh nền gốc bằng các mảng màu chuẩn nền giấy kem `PAPER_BG = (245, 235, 215)` (`#F5EBD7`), căn chỉnh tọa độ chính xác bảo vệ hình vẽ nhân vật hoạt hình.
